...
Begin VB.Form frmPackageReplace 
   BorderStyle     =   0  'None
   Caption         =   "Cellphone POS"
   ClientHeight    =   7215
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   11940
   ...
   Begin xrControl.xrFrame xrFrame1 
      ...
      Begin xrGridEditor.GridEditor GridEditor1 
         ...
         GRIDLINESFIXED  =   2
         GRIDLINEWIDTH   =   1
         MOUSEICON       =   "frmPackageReplace.frx":0000
         MOUSEPOINTER    =   0
         REDRAW          =   -1  'True
         RIGHTTOLEFT     =   0   'False
         ROWS            =   2
   ...
   Begin xrControl.xrButton cmdButton 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Picture         =   "frmPackageReplace.frx":001C
   End
   Begin xrControl.xrButton cmdButton 
      Height          =   600
      Index           =   0
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Picture         =   "frmPackageReplace.frx":0796
   End
   Begin xrControl.xrButton cmdButton 
      Height          =   600
      Index           =   2
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Picture         =   "frmPackageReplace.frx":0F10
   End
   Begin xrControl.xrButton cmdButton 
      Height          =   600
      Index           =   3
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Picture         =   "frmPackageReplace.frx":168A
   End
End
Attribute VB_Name = "frmPackageReplace"
...
Private Const pxeMODULENAME = "frmPackageReplace"

Private p_oAppDrivr As clsAppDriver
Private WithEvents oCPSales As clsCPPOSPackage
Attribute oCPSales.VB_VarHelpID = -1
...
Property Set CPSales(loCPSales As clsCPPOSPackage)
   Set oCPSales = loCPSales
End Property
         ...
         If .Rows > 2 Then
            ...
            Do While pnCtr < .Rows
               If Trim(.TextMatrix(pnCtr, 1)) = "" Then
                  .Row = pnCtr
                  If oCPSales.DeleteDetail(.Row - 1) Then .DeleteRow
               Else
                  pnCtr = pnCtr + 1
               End If
            Loop
...
'            If Trim(.TextMatrix(pnCtr, 1)) = "" Then
'               .Row = pnCtr
'               If oCPSales.ItemCount = 0 Then Exit Do
'               If oCPSales.DeleteDetail(.Row - 1) Then
'                  .DeleteRow
'               End If
'            Else
'               pnCtr = pnCtr + 1
...
'         Loop
         ...
         pbCancelxx = Index = 1
      Case 2
         If oCPSales.SearchDetail(.Row - 1, 1) Then
            .Col = 1
            .Refresh
            .SetFocus
         End If
...
Private Sub GridEditor1_AddingRow(Cancel As Boolean)
   With GridEditor1
      ...
      ElseIf .TextMatrix(.Row, 8) = 0 Then
         Cancel = True
      End If
      If Not Cancel Then oCPSales.AddDetail
...
Private Sub GridEditor1_EditorValidate(Cancel As Boolean)
   ...
   With GridEditor1
      Select Case .Col
      Case 4
         If oCPSales.Detail(.Row - 1, "nQtyOnHnd") <= 0 Then
            If .TextMatrix(.Row, 1) <> "" Then
      ...
      If .Col = 6 Then
         oCPSales.Detail(.Row - 1, .Col) = CDbl(lnDiscount)
      Else
         If .Col = 1 Or .Col = 2 Then
            'If pbSearch = False Then
            oCPSales.Detail(.Row - 1, .Col) = .TextMatrix(.Row, .Col)
         Else
            oCPSales.Detail(.Row - 1, .Col) = CDbl(.TextMatrix(.Row, .Col))
         End If
      End If
...
'         If Not pbGridFocus Then Exit Sub
         ...
         Case 1, 2
            'pbSearch = True
            If oCPSales.SearchDetail(.Row - 1, .Col, .TextMatrix(.Row, .Col)) Then
               If oCPSales.Detail(.Row - 1, 3) <= 0 Then
                  If oCPSales.Detail(.Row - 1, "cLaborxxx") = 0 Then
                     MsgBox "No Stock is Currently Availble!!!", vbCritical, "Warning"
                  End If
               End If
               ...
               '.ColEnabled(5) = False
               'If oCPSales.Detail(.Row - 1, "cPartType") = 0 Then .ColEnabled(5) = True
            End If
            pbSearch = False
         End Select
...
Private Sub oCPSales_DetailRetrieved(ByVal Index As Integer)
   With GridEditor1
      .TextMatrix(.Row, Index) = oCPSales.Detail(.Row - 1, Index)
      If Index = 6 Then
         MsgBox .TextMatrix(.Row, Index)
...
Private Sub oCPSales_MasterRetrieved(ByVal Index As Integer)
   txtField(Index).Text = oCPSales.Master(Index)
End Sub
...
'      Select Case pnCtr
'      Case 0
'         txtField(pnCtr).Text = Format(oCPSales.Master(pnCtr), "@@@@-@@@@@@")
'      Case 1
'         txtField(pnCtr).Text = Format(oCPSales.Master(pnCtr), "MMMM DD, YYYY")
'      Case 5, 6
'         txtField(pnCtr).Text = Format(oCPSales.Master(pnCtr), "#,##0.00")
'      Case 8
'         txtField(pnCtr).Text = Format(p_oappdrivr.LogName, ">")
'      Case Else
'         txtField(pnCtr).Text = IIf(IsNull(oCPSales.Master(pnCtr)), "", oCPSales.Master(pnCtr))
'      End Select
'   Next
...
'      .SystemCd = "SP"
      ...
      If .CheckAmount > 0 Then
         .Checks(0) = oCPSales.Checks(0)
         .Checks(1) = oCPSales.Checks(1)
         .Checks(2) = oCPSales.Checks(2)
         .Checks(3) = oCPSales.Checks(3)
         .Checks(4) = oCPSales.Checks(4)
      End If
...
'      txtField(13).Text = Format(CDbl(oReceipt.TranTotal), "#,##0.00")
      ...
      oCPSales.Master("nAmtPaidx") = oReceipt.CashAmount + oReceipt.CheckAmount
      txtField(7).Text = Format(oCPSales.Master("nAmtPaidx"), "#,##0.00")
   ...
   With txtField(Index)
      ...
      Case 7
         ...
         Else
            ...
            oCPSales.Master(Index) = CDbl(.Text)
         End If
      Case Else
         If Index = 2 Then
            oCPSales.Master(Index) = Format(.Text, ">")
         Else
            oCPSales.Master(Index) = CDbl(.Text)
         End If
      End Select
...
Private Sub LoadDetail()
   ...
   Dim lnSubTotal As Currency
      ...
      With GridEditor1
         .Rows = IIf(oCPSales.ItemCount = 0, 2, oCPSales.ItemCount + 1)

         For pnCtr = 0 To oCPSales.ItemCount - 1
            For lnCtr = 1 To .Cols - 2
               .TextMatrix(pnCtr + 1, lnCtr) = oCPSales.Detail(pnCtr, lnCtr)
            Next
            .TextMatrix(pnCtr + 1, 6) = oCPSales.Detail(pnCtr, 6) & "%"
            lnSubTotal = (oCPSales.Detail(pnCtr, 4) * oCPSales.Detail(pnCtr, 5))
            .TextMatrix(pnCtr + 1, 8) = lnSubTotal - (lnSubTotal * _
                                      (oCPSales.Detail(pnCtr, 6) / 100) - oCPSales.Detail(pnCtr, 7))
         Next
      End With
